...
# ----------------------------
# Custom Pendulum environment with randomized initial state
# ----------------------------
class RandomizedPendulum(PendulumEnv):
    def __init__(self):
        super().__init__()
        self.spec = gym.envs.registration.EnvSpec("RandomizedPendulum-v1", max_episode_steps=200)
        self.action_space = Discrete(5)
        self._torque_map = np.linspace(-2.0, 2.0, 5)

    def reset(self, *, seed=None, options=None):
        super().reset(seed=seed)
        theta = self.np_random.uniform(low=-np.pi, high=np.pi)
        theta_dot = self.np_random.uniform(low=-2.0, high=2.0)
        self.state = np.array([theta, theta_dot], dtype=np.float32)
        return np.array(self._get_obs(), dtype=np.float32)

    def step(self, action):
        continuous_action = np.array([self._torque_map[action]])
...
env_name = "RandomizedPendulum-v1"
register_env(env_name, lambda config: RandomizedPendulum())

print(f"\n[INFO] Starting DQN training on {env_name} with RANDOMIZED INITIAL STATES + DISCRETE ACTIONS...\n")
...
df = pd.DataFrame(log)
df.to_csv("pendulum_inference_DQN_log.csv", index=False)
print("Inference log saved to pendulum_inference_DQN_log.csv\n")

# ----------------------------
# Step 3: Plot
# ----------------------------
plt.plot(df["step"], df["cos(theta)"], label="cos(theta)")
plt.plot(df["step"], df["theta_dot"], label="theta_dot")
plt.xlabel("Step")
plt.ylabel("State Value")
plt.title("State Evolution - Pendulum-v1 using DQN")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("DQN-Pendulum-v1.png")
plt.show()
print("State plot saved to DQN-Pendulum-v1.png")
